...
app.get("/", async (req, res) => {
  ...
  const gameWeekMatches = await fetch(
    `http://api.football-data.org/v4/competitions/2021/matches?matchday=${gameWeek}`,
    {
      headers: {
        "X-Auth-Token": apiToken,
      },
...
app.post("/teamMatches", async (req, res) => {
  const response = await fetch(
    `https://api.football-data.org/v4/teams/${req.body.id}/matches?status=SCHEDULED`,
    {
      headers: {
        "X-Auth-Token": apiToken,
      },
